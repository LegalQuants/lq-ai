...
- **Identify quoted spans:** extract quotation-mark-delimited spans from the assistant answer (the case-law-research skill instructs the model to quote opinions verbatim in quotes). This is caselaw-specific extraction — it does **not** reuse `extract_citations`, which resolves KB `(Source: [N])` markers to retrieved chunks; caselaw quotes carry no `[N]` marker pointing at an opinion.
- **Locate each span:** search each consulted opinion's `normalized_content` for the quoted string (exact substring first, then a normalized/whitespace-tolerant search — the same normalization the cascade's stage-2 uses), yielding `source_offset_start/end` into that opinion's plaintext.
- A quoted span that matches no consulted opinion yields **no** caselaw citation (the model paraphrased or quoted something it didn't actually read).
