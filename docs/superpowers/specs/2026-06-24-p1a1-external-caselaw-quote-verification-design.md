...
- **Locate each candidate:** search every consulted opinion's `normalized_content` for the candidate string — exact substring first, then a whitespace/case-tolerant search (the same normalization the cascade's stage-2 uses) — yielding `source_offset_start/end` into the matched opinion's plaintext. Attribution is **by which consulted opinion the span matches** (a verbatim span matches exactly one); the `### Case Name` header is an optional confirmation hint, not required for v1.
