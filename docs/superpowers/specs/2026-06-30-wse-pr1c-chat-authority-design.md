...
Body (mirrors `caselaw.verify_and_persist_caselaw_citations`, verbatim+paraphrase pass only):
...
3. Load each ref's body once: `body = await load_authority_text(db, source_type=r.provider, external_ref=r.external_ref)`. Cache-miss (`None`) → skip that ref (search-only refs and expired bodies drop naturally). De-dupe by `(provider, external_ref)`. Build `authority_target(r.provider, r.external_ref, body)` per loaded ref. On per-ref load exception → log + skip.
4. Verbatim + paraphrase, first-match-wins, **drop-on-miss**: for each passage, for each loaded target, `off = locate_passage(passage, target.normalized_content)`; on hit build `_AuthorityCandidate` and `result = await verify(cand, target, gateway=gateway, judge_model=judge_model)`; if `result.verified`, append `MessageAuthorityCitation(source_type=<provider>, content_kind=<record.source_kind>, external_ref=<record.external_ref>, source_offset_start=start, source_offset_end=end, source_text=passage, verified=True, verification_method=result.method, verification_confidence=result.confidence, partial=result.partial)` and `break` (first matching authority wins). No match against any target → **no row**.
5. `db.add_all(rows)`; `await db.flush()`; return `len(rows)`.

Notes:
- `verify(gateway=gateway, judge_model=judge_model)` matches PR1b's `build_authority_citations:430` — exact/tolerant short-circuit, paraphrase judge only on near-miss. Passing `gateway=None` disables the judge (verbatim-only) for tests that want determinism.
- No FAIL rows in the chat path (unattributed blockquotes) — divergence from `build_authority_citations`, which FAILs on locate-miss because it has explicit `(quote, source)` evidence pairs. Documented in §5.
- Import `ToolSourceRecord` the same way `caselaw.py` does (avoid a `tool_loop` ↔ `citation` import cycle; use the existing shared import path / `TYPE_CHECKING` if `caselaw.py` does).
...
- **Unit — `authority.py`:** `verify_and_persist_authority_citations` — verbatim PASS (`exact_match`); paraphrase SUPPORTED (`paraphrase_judge` via a stub gateway); drop-on-miss (blockquote matches no body → 0 rows); cache-miss ref skipped; per-ref load exception non-fatal; `gateway=None` verbatim-only determinism.
...
- **Honest verification:** a quote that matches a fetched body verbatim/paraphrase → PASS/SUPPORTED; a quote matching **no** fetched body → dropped (no row), because chat blockquotes carry no reliable authority attribution. No false-FAIL on uploaded-doc/caselaw quotes. (Attributed-authority FAIL = DE-370.)
...
- **Verification depth:** Level 2 — verbatim + paraphrase, PASS/SUPPORTED, **drop-on-miss** (no chat-FAIL); matches the autonomous path's `verify(gateway=…)` call. Attributed-FAIL deferred to DE-370.
