...
- **D-c4** The api reads the non-secret OAuth config (`oauth_client_id`, server canonical URI) from the gateway's **existing** sanitized `GET /admin/v1/config` (no new gateway endpoint). The only gateway change is adding the `oauth_client_id` field to `MCPServerConfig`.
...
- [ ] `async def discover_endpoints(server_url) -> OAuthEndpoints` — `GET {server_url}/.well-known/oauth-protected-resource` (RFC 9728) → take `authorization_servers[0]` + `resource` (canonical URI); then `GET {as}/.well-known/oauth-authorization-server` (RFC 8414), falling back to `/.well-known/openid-configuration` — extract `authorization_endpoint`, `token_endpoint`, `issuer`, `authorization_response_iss_parameter_supported`, `scopes_supported`. Return a small dataclass. Use a plain `httpx.AsyncClient` (these are public metadata GETs to the MCP server's own host — NOT gateway-brokered, because they're unauthenticated discovery of the AS, not tool egress; document this exception to ADR 0014 explicitly, OR route through a gateway discovery passthrough if the maintainer prefers — FLAG in review). Cache per-process with a TTL.

> ⚠️ **Open point for review (ADR 0014):** the OAuth *discovery + token-exchange* HTTP calls go to the MCP server / its auth server. Under ADR 0014 all third-party egress goes through the gateway. Options: (a) the api makes these auth-only calls directly (they carry no matter data — only OAuth metadata + the user's auth code/token), documented as a scoped exception; (b) add a gateway passthrough for the OAuth endpoints. This plan assumes (a) with a clear comment; **confirm with the maintainer** — if (b), add a gateway `POST /v1/oauth/{provider}/{token|discover}` and route through it.
...
- **ADR-0014 discovery-egress exception** (Task 4b ⚠️) — direct api OAuth calls vs gateway passthrough. Maintainer call.
