...
description: Day-to-day product help lives inside the running application, not on this site — with one exception.
audience: [operator, evaluator]
status: draft
sources:
  - README.md
  - web/src/lib/lq-ai/tabs.ts
sidebar:
  order: 6
---
...
A small set of situations carry a duty beyond "how do I use the product" — a leaked provider key, a security vulnerability you've found, a control that failed silently on privileged work. Those procedures live on this site, in [Operate](../operate/index.md) and [Trust](../trust/index.md), because they're read under time pressure by someone who may not be signed into the deployment that's the problem, and because the professional-conduct stakes (client notification, confidentiality, disclosure obligations) belong in a durable, citable place rather than inside an app session:

- [Rotate a leaked provider key](../operate/rotate-a-leaked-key.md) — revoke, replace, and evidence the blast radius.
- [Security disclosure](../trust/security-disclosure.md) — what to do with a vulnerability you've found, and where the line to the public tracker sits.

## Next

- [Choose your path](choose-your-path.md) — route to the namespace for what you actually need
- [Operate](../operate/index.md) — day-to-day running of a deployment
- [Trust](../trust/index.md) — the incident and disclosure procedures above, in context
