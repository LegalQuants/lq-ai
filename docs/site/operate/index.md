---
title: Run and maintain LQ.AI
description: Install LQ.AI, plan the maintenance work that follows, and know what to do when something goes wrong — indexed by hosting shape.
...
This is the operator's namespace: getting LQ.AI running, keeping it running, and knowing what to do when it doesn't. Docker Compose is the reference installation, tested end to end on this site; start there, then come back for the week-two work once it's up.

## Choose an installation

Five install paths have a recipe today. Pick the one that matches how you plan to run LQ.AI — each page names what it covers and what it doesn't. Windows and corporate-cloud-specific deployment (running the stack itself on Azure, AWS, or GCP infrastructure) are not covered by a recipe yet; the gateway's [Azure OpenAI provider adapter](../../../gateway/app/providers/azure_openai.py) is a model provider you can point the stack at, not a guide to hosting the app on Azure.
...
- **[Kubernetes via Helm](../../operate/install-helm.md)** — a chart for the core chat services; it omits some background workers the Compose setup runs. Read the page before you rely on it for more than that.
...
- [Is LQ.AI right for your team?](../../start/is-it-for-you.md)
