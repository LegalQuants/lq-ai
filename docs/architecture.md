...
**Self-hosted, with the customer's keys, in the customer's environment.** Storage (Postgres + Redis + RustFS/S3) lives entirely in the operator's environment; the application services run alongside. Customer *content* leaves the operator's environment only through the Inference Gateway — the inference call to the configured provider and, where an operator has enabled them, gateway-brokered tool calls (case-law lookup, MCP connectors). The gateway is the routing boundary for content, not a stack-wide egress firewall: a few other paths need network access and should be reviewed on their own before a deployment is described as offline — image builds; the ingest worker's first-run download of Docling's models from Hugging Face (`ingest-hf-cache` in `docker-compose.yml`; the pass is still shipped pending its removal under ADR 0026, and `LQ_AI_DOCLING_ENABLED=false` skips it in air-gapped environments); optional SMTP email for autonomous notifications (`smtp_host`, off by default; carries counts, IDs and a receipt link, not content); and OpenTelemetry export (`OTEL_EXPORTER_OTLP_ENDPOINT`, off by default). There is no LegalQuants-side SaaS holding customer data — by design, no telemetry is emitted by default, and what telemetry the operator opts into contains no content. (See [PRD §5.7 No Telemetry by Default](PRD.md#57-no-telemetry-by-default).)
...
| Files (uploaded documents) | The configured object store — the bundled RustFS by default, or whatever S3-compatible endpoint `S3_ENDPOINT_URL` points at | The original bytes: no, provided the object store itself is local. The *extracted text* is a separate question: attached-file and retrieved-KB text is injected into inference requests verbatim (marked to skip anonymization, so the model sees intact source text) and so departs per the inference-payload row below |
| Document chunks (post-ingestion) | PostgreSQL (pgvector + FTS) | The stored chunks: no. Computing their vectors sends chunk text to whichever provider backs the gateway's `embedding` alias — `openai-prod/text-embedding-3-small` in `gateway.yaml.example` — independently of the chat model selected, so a local chat model does not by itself keep embedding local |
...
| Inference request payloads | Travel through the Inference Gateway to the configured provider | **Yes**, per the routed Inference Tier — see [PRD §1.5.2](PRD.md#15-deployment-modes-and-the-inference-choice-spectrum) for what each tier is intended to represent about provider-side retention and training. The tier is a classification the operator declares per provider in `gateway.yaml` (`tier: 4` for the standard commercial API, `tier: 3` if a ZDR agreement is in place); the gateway enforces that classification, it does not verify the provider's actual agreement, account plan or region |
...
- **The Web App box holds a second backend.** The `web` image is an OpenWebUI fork, and OpenWebUI ships its own embedded Python backend inside that same image — separate from LQ.AI's `api` service — serving `/api/config`, the auth shell, and the websockets the SPA needs on boot; LQ.AI's own workspace calls go to the separate `api` service instead (see [`web/docs/frontend-dev.md`](../web/docs/frontend-dev.md)). Both backends mount an `/api/v1`: the release stack's bundled proxy resolves the collision by giving LQ.AI's `api` its own `/lq`-prefixed origin and stripping that prefix back before forwarding (`docker-compose.release.yml`). Keep the two sets of URLs, sign-in paths, and proxy routes distinct when debugging the app or exposing it on another host — see [Reverse proxy and TLS](operate/reverse-proxy-tls.md).
