...
| **P1 authoritative content** | **Partial.** Gateway tool-provider class (ADR 0014); CourtListener wired (WS3); MCP client lets operators connect their own sources (WS2). | Expand free sources (GovInfo/EUR-Lex/SEC EDGAR — DE-280/281); a **content-source registry** so the agent knows which authoritative sources exist, their jurisdiction + egress tier; the BYO-licensed-corpus framing. |
...
- **[ADR 0018 — The Citation Ledger & fiduciary-grade output](../adr/0018-citation-ledger-and-fiduciary-grade-output.md)** (WS-A/B) — **drafted (proposal, 2026-06-24)**: the thin referencing data model (D1), external-source quote-verification via materialized citable sources (D2), the fiduciary-grade gate (D3), the one-click trace read model (D4), the no-raw-payload guarantee (D5), the reserved treatment slot (D6), and retention (D7). Pending maintainer + security acceptance (gates Phase 1 code).
...
**P1-B1 — Fiduciary-grade gate (WS-B, ADR 0018 D3).** Compute the gate at finalize — PASS `{exact, tolerant}`; SUPPORTED `{paraphrase, ensemble}` labeled distinctly; FAIL `{unverified, failed}` flagged inline, never silently dropped (conservative posture). Record the verdict against `work_product`; include in the P9 export. Reuse `TOLERANT_MATCH_THRESHOLD` / aggregation ([DE-281](../PRD.md#de-281)); cost pre-flight for long-opinion verification.
- *Files:* chat finalize path, `work_product` linkage, config thresholds, tests.
- *Acceptance:* all-PASS draft → `fiduciary_grade=true`; one unverifiable claim → `false` + inline flag; verdict recorded + exported; "supported, not verbatim" never presented as a verbatim quote.
...
- **[DE-344](../PRD.md#de-344)** (external-tool cost is `Decimal(0)`) → **P1-A1/P1-B1** add the long-opinion verification cost pre-flight that begins to close it.
...
- Deferred items in scope: [DE-200](../PRD.md#de-200) (MCP — landed), DE-279/280/281 (additional sources), DE-344 (per-provider external-tool cost).
