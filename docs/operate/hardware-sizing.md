...
> [!CAUTION]
> **Before you run this** — Below the RAM floor, the failure is not "runs slower" —
> it's a boot failure. README's own troubleshooting section: *"`docker compose up`
> fails immediately — confirm Docker Desktop is running and has at least 6 GB RAM
> allocated."* On macOS: Docker Desktop → Settings → Resources. There is no in-app
> signal beyond the compose error itself; a container that never starts doesn't
> leave a growth curve to read.
...
The first `docker compose up -d` also pulls container images across those eight
services; on a reasonable connection this is minutes, not the steady-state disk
footprint. A related, larger number from a different install path: the macOS desktop
launcher's first run downloads roughly 17 GB of images (`api` ~9.5 GB, `web` ~6 GB,
`gateway` ~1.6 GB, plus `proxy`) before it starts pulling document-processing models
— [docs/PRD.md](../PRD.md) records this as the reason that first run can look frozen
for 10–30 minutes on a slow connection. It is not the same number as the compose ~8
GB floor above (different install path, different image set), and it isn't
reproduced here as a general Compose expectation — cited so a slow first pull doesn't
read as a hang.
...
No named configuration for Kubernetes/Helm resource requests beyond the per-component
`requests`/`limits` in [`deploy/helm/lq-ai/values.yaml`](../../deploy/helm/lq-ai/values.yaml)
(which are defaults, not a sizing recommendation), no per-seat or per-matter sizing,
and no benchmark of tokens-per-second on any hardware. If your plan needs one of
those, this page's honest answer is: not documented in the repository as of the
checked commit — measure it yourself against your own workload before you commit
hardware to it.
