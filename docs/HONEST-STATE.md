...
| AWS Bedrock (Mantle) provider adapter — Responses tier | shipped, live-verified against `openai.gpt-oss-20b` (general `/v1/responses` path), `google.gemma-4-31b` and `xai.grok-4.3` (both use the `openai/v1/responses` exception path per their AWS model cards). `openai.gpt-5.4`/`gpt-5.5` are documented to use the same exception path but remain unverified — live calls return 401 `permission_denied_error` (account model entitlement, not a code/URL issue) | `gateway/app/providers/bedrock_mantle.py` ([DE-035](PRD.md#9-deferred-enhancements-and-identified-future-work)) |
| AWS Bedrock (Mantle) provider adapter — Messages tier (current-gen Anthropic models) | implemented and unit-tested against documented schema; live 200-OK verification blocked on AWS account model entitlement (403 `permission_error` for both `anthropic.claude-opus-4-8` and `anthropic.claude-haiku-4-5`), not on missing code | `gateway/app/providers/bedrock_mantle.py` ([DE-035](PRD.md#9-deferred-enhancements-and-identified-future-work)) |
