...
| AWS Bedrock (Mantle) provider adapter — Chat Completions tier | shipped, live-verified against `openai.gpt-oss-20b` and `zai.glm-5` | `gateway/app/providers/bedrock_mantle.py` ([DE-035](PRD.md#9-deferred-enhancements-and-identified-future-work)); `pytest -m provider tests/test_bedrock_mantle_adapter.py` |
| AWS Bedrock (Mantle) provider adapter — Responses tier | shipped, live-verified against `openai.gpt-oss-20b`, `google.gemma-4-31b`, and `xai.grok-4.3`. `openai.gpt-5.4`/`gpt-5.5` are documented ([GPT-5.4](https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-openai-gpt-54.html), [GPT-5.5](https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-openai-gpt-55.html) model cards) to use the same URL path as Gemma/Grok, and returned a 401 "model not available for this account" error, tested using a personal AWS account with very low activity — likely entitlement, may require an AWS Sales Support request | `gateway/app/providers/bedrock_mantle.py` ([DE-035](PRD.md#9-deferred-enhancements-and-identified-future-work)) |
| AWS Bedrock (Mantle) provider adapter — Messages tier (current-gen Anthropic models) | implemented and unit-tested against documented schema; live 200-OK verification blocked by a 403 "model not available for this account" error, tested for `anthropic.claude-opus-4-8`, `anthropic.claude-opus-4-7`, `anthropic.claude-haiku-4-5`, and `anthropic.claude-sonnet-5`. Likely account entitlement — tested using a personal AWS account with very low activity, may require an AWS Sales Support request to enable. Message-tier translation reuses `app/providers/anthropic.py`'s existing, already-live-tested functions directly — see AWS's [Claude Opus 4.8 model card](https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-anthropic-claude-opus-4-8.html), which documents `bedrock-mantle` Messages support at exactly the URL this adapter uses | `gateway/app/providers/bedrock_mantle.py` ([DE-035](PRD.md#9-deferred-enhancements-and-identified-future-work)) |
