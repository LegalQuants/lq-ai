# Handoff — M4 / LQVern execution: resume at Task M4-A4
...
> **Where we are:** the M4 design is approved + on `main`; the implementation plan is written; **Phase A is nearly complete** — M4-0.1, M4-A1, M4-A2, and **M4-A3 (the load-bearing chokepoint, all four sub-dispatches) are DONE and pushed**. **Resume at Task M4-A4** (halt API + idle-halt watchdog cron + per-session receipt + sessions read API) — the LAST Phase-A task before the hard gate to Phase B. Branch HEAD `52cfd1a`.
...
>
> **M4-A4 quick-start (per the plan, Task M4-A4):** three endpoints (`POST /api/v1/autonomous/sessions/{id}/halt` → sets `halt_state='halt_requested'`, idempotent, 404 on another user's id; `GET /api/v1/autonomous/sessions` paginated newest-first; `GET /api/v1/autonomous/sessions/{id}` + full receipt) + arq cron `autonomous_idle_watchdog` (register in `WorkerSettings.cron_jobs`, mirror `document_pipeline._build_cron_jobs`; `running→paused→halted` across two idle ticks past `idle_halt_minutes`, writes `halted` audit reason `idle_timeout`) + `api/app/autonomous/receipt.py::build_receipt(session, db) -> dict` (per-tool phase/intent/cost/outcome/gates-passed + transitions + terminal reason; **no raw entity values** — extend the A3.3b privacy-guard assertion to the receipt) persisted into `autonomous_sessions.result`. New endpoints get unit + integration + **OpenAPI-conformance** tests; update `docs/api/backend-openapi.yaml`. The halt→next-tool-call-halts integration ties to the A3.3a R5 brake. **Watch the DELETE/204 FastAPI pitfall if any endpoint 204s** (CLAUDE.md). Reuse `app.autonomous.audit.autonomous_audit` for the `halted` row.
