...
**M3 status: PLUMBING SHIPPED (v0.3.0); M3-B3 (chat) landed as a community contribution — remaining feature surface still deferred (M4 closed without it; now M4+ / community per DE-287).** Scaffold (M3-B1) + OAuth via Office.js Dialog API (M3-B2) + self-served bundle + version handshake (M3-B8) all shipped. The `word-addin/` directory holds the React 18 task pane, Office.js XML manifest template, admin manifest-generation endpoint (`GET /api/v1/admin/word-addin/manifest`), and the version-handshake endpoint (`GET /api/v1/word-addin/version`); the unsigned-manifest sideload path via Microsoft 365 Admin Center is the v0.3.0 install path. Implementation companion: [docs/word-addin.md](word-addin.md). Two parallel scope-reductions land at v0.3.0:

- **Feature surface inside the task pane.** M3-B3 (document-grounded chat) shipped per [§9 DE-287](#de-287--word-add-in-feature-surface-chat-skills-playbooks-tier-badge--deferred-to-m4--community-contribution) — see that entry for the honest scope delta against the original task description (non-streaming; click-to-scroll citations, not the full 5-state Citation Engine UI). Skills with tracked-changes + comments rendering (M3-B4), playbook execution (M3-B5), and the Inference Tier badge (M3-B6) remain descoped to M4 / community contribution; those tabs still render a deep-link card to the equivalent web-app surface.
...
- **M3-B3 — Chat against the open document. SHIPPED** (community PR against `feat/word-addin-chat` — cross-reference PR number once merged). Landed with a narrower scope than originally written above, worth stating plainly rather than letting the original bullet imply full parity:
  - **New dedicated endpoint, not `/api/v1/chat/completions`.** `POST /api/v1/word-addin/document-chat` (`api/app/api/word_addin.py`) — deliberately stateless (no `db` dependency, nothing written to `chats`/`messages`): the open Word document is the state, not a persisted conversation, so extending the persisted-chat endpoint (as originally scoped) would have embedded document content into cross-session chat history for no benefit.
  - **Non-streaming**, not the originally-scoped streaming responses. A deliberate call, not a gap that was missed.
  - **Structured citations via forced tool-calling** (`emit_answer`, provider-enforced schema conformance), rendered as clickable inline Markdown links (`citation:<n>` convention) that call `docHelper.scrollToParagraph` to select/scroll to the source paragraph in the live document — **not** the full 5-state Citation Engine UI (`exact_match`/`tolerant_match`/`paraphrase_judge`/`ensemble_*`/`unverified` states, §3.3). There is no server-side verification pass: a live, unpersisted Word document has no `documents.normalized_content`-equivalent to verify a citation against, unlike the KB Citation Engine. The click-to-scroll either finds the quoted text in the live document or falls back to selecting the whole paragraph — a weaker, but honest, grounding guarantee than the KB citation engine's byte-level verification.
  - **No selection-only context mode** — only whole-document context shipped; every turn resends the full paragraph snapshot.
  - Open-document context mode, task-pane chat UI, and in-doc paragraph highlighting via Office.js range APIs did ship, matching the original scope.
