...
**M3 status: PLUMBING SHIPPED (v0.3.0) — slash-command surface still deferred (M4 closed without it; DE-288); OAuth E2E unverified.** The `slack-bridge` (M3-D1) and `teams-bridge` (M3-D3) standalone services, their OAuth handlers, the api-side persistence endpoints (bridge-bearer-authed, bot-token encrypted at rest), and the admin management surface (M3-D4) all shipped under the `slack`/`teams` Compose profiles. Implementation companion: [docs/intake-bridges.md](intake-bridges.md). **Honesty caveats:** the `/lq` quick-ask surface (`/lq help` + `/lq ask` on both bridges + the bridge-bearer quick-ask api endpoint) has since shipped ([§9 DE-288](#9-deferred-enhancements-and-identified-future-work)) but — like the OAuth plumbing — is verified **in isolation only**: no real OAuth or slash-command round-trip against live Slack/Microsoft tenants has been exercised ([§9 DE-312](#9-deferred-enhancements-and-identified-future-work), P1), and the teams-bridge messaging endpoint ships without inbound Connector JWT validation (interim posture documented in `teams-bridge/app/commands.py`; DE-288 status note). Do not read this as a claim that the bridges work end-to-end.
...
#### DE-288 — Slack/Teams `/lq` slash command + quick-skill flow — SHIPPED (quick-ask scope); thread-forward + admin dropdown remain

**Priority:** P2 · **Effort:** M (8–10 hours Slack + ~8 hours Teams parity)

**Status update (2026-07-25 — quick-ask scope shipped, not live-verified):** the `/lq help` + `/lq ask "<question>"` flows shipped on both bridges: `POST /slack/commands` (signature-verified via the existing `signing.py` primitive; 3-second ephemeral ack; ONE final `response_url` post) and `POST /teams/messages` (ActivityHandler-shaped dispatch; reply via the Bot Connector `serviceUrl`). Both call the new bridge-bearer api endpoint `POST /api/v1/integrations/quick-ask`, which resolves identity fail-closed (Slack: profile email via `users.info` with the stored workspace bot token — requires the `users:read` + `users:read.email` scopes added to the manifest, so pre-existing installs must re-install; Teams: email from the Connector conversation-member record), maps email → LQ.AI account, and runs the turn through the normal chat-send path as that user (`bridge.quick_ask` audit row with `details.source`, `bridge-{platform}-…` request id into the gateway routing/cost log). The quick-ask skill is operator-configured via `LQ_AI_BRIDGE_QUICK_ASK_SKILL` (env; empty = plain chat turn); the chat deep link uses `LQ_AI_WEB_PUBLIC_URL`. **Still open:** (a) the `/lq` no-arg thread-forward flow described below; (b) the admin-UI dropdown for the quick-ask skill (env-var only today); (c) teams-bridge inbound Connector JWT validation (maintainer fork: minimal PyJWT validator vs `botbuilder-core` — interim posture documented in `teams-bridge/app/commands.py`); (d) live-tenant end-to-end verification (maintainer-held; DE-312). The original scope text is retained below for the remaining pieces.
