...
- **Retrieval ownership scoping (Security Audit (Claude Fable 5): 22 findings across gateway / api / web / supply-chain #288 AG-01).** The autonomous chokepoint's `retrieve_chunks` now verifies the session owner owns the model-supplied `kb_id`/`file_id` before any retrieval, mirroring the HTTP surface's visibility predicates (owner scope + `archived_at`/`deleted_at IS NULL`); foreign or unknown ids fail closed with a 404-shaped error. Closes a cross-user read reachable via prompt-injected planner args.
