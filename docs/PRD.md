...
**Update (2026-07-25): SHIPPED — option (a).** `ledger_bridge.build_authority_citations` now runs the budgeted whole-body paraphrase judge on a locate miss when a gateway is available, reusing chat Pass B's machinery unforked (`authority_content_judge.judge_authority_content`, pre-flight `estimate_authority_content_cost_usd` accounting against the shared `AUTHORITY_CONTENT_JUDGE_BUDGET_USD` per-turn cap). A supporting verdict writes a SUPPORTED row (`verified=True, verification_method="paraphrase_judge", partial=True`, whole-body placeholder offsets `start=0, end=len(body)` exactly like chat's Pass B rows); a non-supporting verdict, judge error, budget exhaustion, or `gateway=None` keeps the fail-closed FAIL row. The judge's ceiling is SUPPORTED — it never upgrades to PASS. The verbatim `verify()` call after a successful locate now passes `gateway=None` (the judge stage is structurally unreachable there), closing the dead-argument honesty gap. Gate/ledger unchanged (`paraphrase_judge` was already in the SUPPORTED bucket).
