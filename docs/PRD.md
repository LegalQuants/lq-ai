...
**M3 status: SHIPPED (v0.3.0).** The `output_format: table` skill mode (M3-C1), the three-node LangGraph Tabular Review workflow (M3-C2), and XLSX/CSV export (M3-C4a) all shipped. Endpoints: `POST /api/v1/tabular/preview-cost`, `POST /api/v1/tabular/execute`, `GET /api/v1/tabular/executions/{id}`, `GET /api/v1/tabular/executions/{id}/export`. Implementation companion: [docs/tabular-review.md](tabular-review.md). **Honesty caveats:** per-cell citations are now *navigable* — `GET /api/v1/tabular/executions/{id}` resolves each cell's grounding chunk read-side to its `source_file_id`, `source_page`, and `source_text` (two batched IN-queries; existing executions are enriched too) over the synthetic `citation_id` bridge, so the UI can jump to source. Per [§9 DE-309](#9-deferred-enhancements-and-identified-future-work) (shipped) the executor now also mints real offset-bearing provenance rows (`tabular_cell_citations`, migration 0066) for cells whose extracted value locates verbatim in a cited chunk; `GET /executions/{id}` serves those real `citation_id`s + offsets, keeping the uuid5 bridge only as a fallback for pre-migration executions. Fail-closed: a cell whose value doesn't locate serves no citations (renders unverified). The enrichment fields are still untyped in the generated API client ([§9 DE-330](#9-deferred-enhancements-and-identified-future-work)). Per-cell `tier_used` + `cost_usd` are not yet captured ([§9 DE-310](#9-deferred-enhancements-and-identified-future-work)); bulk row/column operations are deferred ([§9 DE-304](#9-deferred-enhancements-and-identified-future-work)).

**Per-column ensemble verification (post-v0.4.0, #127).** A column's `ensemble_verification` flag is now honored at execution. When a column resolves to ensemble-on and the gateway has an ensemble configured, each of that column's cells runs one Stage-4 ensemble verify pass (§3.8) over the concatenation of its cited chunks; the resulting `verification_method` (`ensemble_strict`, `ensemble_majority`, or `None`) persists on the cell and is mirrored onto each citation. A near-verbatim single-chunk value can legitimately resolve to `exact_match`/`tolerant_match` instead — a stronger verification, not an error — and a verification failure never fails the cell. Cost preview gains `ensemble_cells_count` and `ensemble_premium_usd` (the premium is included in `estimated_cost_usd`). The effective flag follows the precedence column > skill snapshot > deployment default, resolved identically at preview and execute. Per [§9 DE-331](#9-deferred-enhancements-and-identified-future-work) (shipped) the executor now also enforces a mid-run ensemble cost ceiling: before each cell's ensemble pass, the projected spend (actual extraction spend so far + estimated ensemble spend so far + one more estimated pass, the same rolling-average judge-cost math as the preview premium) is checked against the operator-confirmed `confirmed_cost_usd`; once it would be exceeded, ensemble is skipped for all remaining cells — extraction always continues (degrade-only) and the results payload records `ensemble_halted_at_ceiling` + `ensemble_halted_cells` so the UI can state the degradation honestly. Runs without a confirmed cost have no ceiling.
...
**Status: SHIPPED.** Migration `0066_tabular_cell_citations.py` adds the `tabular_cell_citations` table (one offset-bearing row per (cell, cited chunk) pair; keyed `(execution_id, document_id, column_name)`). The executor locates each grounded cell's extracted value verbatim in its cited chunks' canonical text (`locate_passage` + verification-cascade Stages 1–2 with `gateway=None` — deterministic, no LLM judge) and the aggregate node batch-inserts the rows in the same transaction as the results write. Fail-closed: an unlocatable value mints no row and the cell serves an empty `citations` list (renders unverified) — never a fabricated offset. `GET /api/v1/tabular/executions/{id}` serves the minted `citation_id`s + chunk-local offsets (`source_offset_start/end` resolve into the served `source_text`); the uuid5 bridge is retained ONLY as a per-execution fallback for executions predating the migration, and the export shape is unchanged. Honesty caveats: minting is verbatim-only (a paraphrased cell value yields no provenance row — a tolerant/fuzzy locator is a possible follow-on); the wire fields remain untyped in the generated client (DE-330, still open).
...
**Status: Shipped.** The tabular extraction node (`api/app/tabular/nodes.py`) now checks, before each cell's Stage-4 ensemble pass, the projected spend — actual extraction spend so far (DE-310 per-cell gateway cost annotations) + estimated ensemble spend so far + one more estimated pass (`estimate_ensemble_pass_cost_usd`, the same rolling-average judge-cost primitive as the preview premium) — against the operator-confirmed `confirmed_cost_usd`. Once the projection would exceed the ceiling, ensemble is skipped for all remaining ensemble-eligible cells (sticky halt); extraction always continues (degrade-only, never fails the run) and the results payload records `ensemble_halted_at_ceiling` + `ensemble_halted_cells` (additive, defaulted fields — pre-existing payloads validate unchanged). Runs without a confirmed cost apply no ceiling. Tests: `api/tests/tabular/test_ensemble_ceiling.py`. Degrading to single-judge (rather than no-verify) remains a possible refinement.
...
#### DE-379 — Server-side signed / hash-chained attestation export of the provenance record

**Priority:** P2 · **Effort:** L · **Status (2026-07-03): filed (Donna upstream request, deferred by maintainer — "no for now").**

Donna requested (`Donna/docs/upstream-requests/lq-ai-signed-attestation-export.md`) a **server-side signed export** of the provenance record — the citation ledger + fiduciary gate (+ per-message `content_hash`es) wrapped in a cryptographically verifiable envelope (a JWS/COSE detached signature over canonicalised JSON, and/or a hash-chained Merkle attestation), with a documented verification path (public key / verify endpoint). Today there is **no cryptographic integrity mechanism** on the trail: `WorkProductAttribution.content_hash` is a per-message SHA-256 but is **unsigned and unchained** (Merkle is noted in-code as "future M2+"), and there is no signed (or even canonical) ledger/gate export endpoint. Donna already shipped the honest unsigned precursor (its Slice 4 "Export ▾" labelled "not a cryptographically signed attestation"), built to drop in a signed endpoint with no rework.
...
#### DE-384 — Runtime `DELETE` can remove a keyless *env-configured* tool provider the operator hand-wrote
