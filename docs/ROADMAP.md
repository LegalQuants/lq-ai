...
|---|---|---|---|---|---|---|
| 1.1 | ~~DE-370 — Attributed-authority FAIL tier (chat)~~ **SHIPPED** | [PRD §9 DE-370](PRD.md#de-370--attributed-authority-fail-tier-chat) | 🟡 Medium | M | Mid, Backend + AI/ML | Shipped: a deterministic citation-attribution parser (`api/app/citation/authority_attribution.py`) binds chat blockquotes to nearby statute/reg/CELEX citations; a quote attributed to a fetched authority *X* that isn't verifiable against *X*'s fetched text FAILs and flags the fiduciary gate. Unattributed quotes keep drop-on-miss. |
