...
| 1.2 | DE-371 — Autonomous-path authority SUPPORTED tier (or drop the dead judge argument) | [PRD §9 DE-371](PRD.md#de-371--autonomous-path-authority-supported-tier-or-drop-the-dead-judge-argument) | 🟡 Medium | S–M | Mid, Backend | Shipped (2026-07-25, option (a)): `build_authority_citations` now runs chat's budgeted whole-body paraphrase judge on a locate miss (SUPPORTED ceiling — never upgrades to PASS; fail-closed FAIL default), and the verbatim `verify()` call passes `gateway=None` (dead-argument cleanup). The `sec_filing` content-kind fix landed earlier in WS-E PR2a. |
