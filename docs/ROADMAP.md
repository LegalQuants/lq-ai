...
| 1.7 | **M4-D2 — Boundary-registers flip + docs finalization + fresh-install acceptance** | [M4 §M4-D2](M4-IMPLEMENTATION-PLAN.md) | 🟡 Medium | M | Mid, Backend + Docs | The acceptance lap that flips PRD §3.10 status to "Shipped (v0.4.0)" and runs a clean-stack walkthrough of every M4 surface. |
...
| 2.2 | **DE-288 — Slack/Teams `/lq` slash command + `/lq ask` quick-skill flow** | [PRD §9 DE-288](PRD.md#de-288--slackteams-lq-slash-command--quick-skill-flow--deferred-to-m4--community-contribution); [M3 §M3-D2/D3](M3-IMPLEMENTATION-PLAN.md) | 🟡 Medium | L | Mid, Backend (`slack-bridge`/`teams-bridge` services) | `/lq help` + `/lq ask` (quick-skill) shipped on both bridges with the bridge-bearer quick-ask api endpoint (fail-closed email identity mapping). What remains: the `/lq` no-arg thread-forward flow, the admin UI dropdown for the quick-ask skill (env-var `LQ_AI_BRIDGE_QUICK_ASK_SKILL` today), teams inbound JWT validation (maintainer fork), and live-tenant E2E verification (DE-312). |
