...
lq_ai:
  ...
  tool_usage:               # optional; list of connector identifiers this skill uses
    - courtlistener
...
- **`lq_ai.tool_usage`** — list of connector identifiers the skill relies on (e.g., `[courtlistener]`). Parsed by the api and surfaced on the skill-detail page: each identifier is checked against the operator's configured connectors, and the UI notes which are available and which are not. **Not enforced** — a skill whose connector is not configured still loads and runs; the operator sees a non-blocking advisory note rather than a hard refusal. Omit this field (or set it to an empty list) for skills that use no external connectors. Use it for any skill that calls a specific MCP server or research provider so that operators can see at a glance what infrastructure the skill needs.
