...
**Slack / Teams Light Intake Bridge (M3, plumbing shipped).** Standalone `slack-bridge` + `teams-bridge` services (opt-in Compose profiles) with OAuth install + identity binding + an admin management surface. The `/lq help` + `/lq ask` quick-skill surface is implemented on both bridges with fail-closed email identity mapping ([DE-288](docs/PRD.md#9-deferred-enhancements-and-identified-future-work); the `/lq` thread-forward flow and the admin dropdown remain open); no real OAuth or slash-command round-trip against live Slack/Microsoft tenants has yet been exercised end-to-end ([DE-312](docs/PRD.md#9-deferred-enhancements-and-identified-future-work)), and the Teams messaging endpoint's inbound JWT validation is a documented pending follow-up. Light intake, deliberately not full triage/SLA/approvals — that is the dedicated legal-intake/triage-platform category. See [`docs/intake-bridges.md`](docs/intake-bridges.md).
