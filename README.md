...
LQ.AI ships with five provider adapters: **Anthropic** (Claude), **OpenAI** (GPT models + text-embedding-3-*), **Azure OpenAI** (M2-E1 — unblocks Azure-tenant enterprise deployments via the operator's existing Azure agreement), **AWS Bedrock** (DE-035 — via AWS's Bedrock Mantle endpoint, a single Bearer-token auth across three OpenAI/Anthropic-native wire protocols, with a hardware-attested Zero Operator Access custody guarantee), and **Ollama** (local). Any OpenAI-compatible local endpoint (vLLM, llama.cpp wrappers, LM Studio) works through the OpenAI adapter with a custom `base_url`. Google Vertex AI is on the deferred-enhancement list (DE-034) and welcomes community contribution — see [docs/HONEST-STATE.md](docs/HONEST-STATE.md) for the current shipped-vs-deferred catalog.
