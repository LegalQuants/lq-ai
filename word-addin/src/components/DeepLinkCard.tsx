...
export const DeepLinkCard: React.FC<DeepLinkCardProps> = ({ title, body, href }) => {
  return (
    <section className="lq-card" role="tabpanel" aria-labelledby={`lq-tab-active`}>
      <h2 className="lq-card-title">{title}</h2>
      <p className="lq-card-body">{body}</p>
      <a className="lq-card-cta" href={href} target="_blank" rel="noopener noreferrer">
