...
import type { AuthUser } from "@/taskpane/auth";
import { Group, Title, Text, Pill, Button } from "@mantine/core";
...
export const Header: React.FC<HeaderProps> = ({ deploymentOrigin, user, onSignOut }) => {
  const originHost = (() => {
    try {
  ...
  return (
    <Group role="banner" bg="sage" justify="space-between">
      <Group gap={2}>
        <Title order={1}>LQ</Title>
        <Text size="sm" children={originHost} />
      </Group>
      <Group>
        {user && <Text children={userLabel} />}
        <Pill children="Tier" c="sage" />
        <Button size="xs" onClick={onSignOut} aria-label="Sign out of LQ.AI" children="Out" />
      </Group>
    </Group>
  );
};

export const Header_DEP: React.FC<HeaderProps> = ({ deploymentOrigin, user, onSignOut }) => {
  const originHost = (() => {
    try {
      return new URL(deploymentOrigin).host;
    } catch {
